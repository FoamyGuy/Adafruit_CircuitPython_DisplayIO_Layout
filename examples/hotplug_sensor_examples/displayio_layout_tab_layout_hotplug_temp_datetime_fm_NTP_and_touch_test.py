...
2022-05-07 Added function refresh_from_NTP().
           ...
           This is done because I discovered that the internal clock of this microcontroller
           is running ahead of time after a while.
NOTE that the touch interface will only start to work after 'loop started' is printed in REPL
           and from the moment the datetime stamp is printed in REPL
           This is done to give precedence to @foamyguy's initial demo steps of setting up
           the Tabs and moving from some Tab to another tab.
           This 'late' becoming available of the touch functionality can easily be improved
           through bypassing the demo steps from @foamyguy.
           In total it takes 53 seconds after Powering up the PyPortal and the first print
           of a datetime stamp in REPL
           Added a label2 to page1, indicating 'Please wait...' at startup and 'Ready...'
           at start of loop. This label is set to "" after a first touch event.
"""
# pylint: disable-all
import time
import gc
...
use_ntp = True
ntp_refresh = False  # at hour change we will re-sync datetime
#                      from NTP server (the internal clock
...
sDT_old = ""
nHH_old = -1  # Used to check if the hour has changed. If so we have to re-sync
#    from NTP server (if not using an external RTC)
...
another_text = Label(terminalio.FONT, text="And another thing!", scale=2, color=0x00ff00, \
    anchor_point=(0, 0),
...
def get_dt():
    global rtc_present, rtc, pge3_lbl, pge3_lbl2, pge3_lbl3, o_secs, c_secs, dt_refresh, sDT_old
    dt = None
    RetVal = False
...
def main():
    global temp_in_REPL, ntp_refresh  # , start_time
    cnt = 1
