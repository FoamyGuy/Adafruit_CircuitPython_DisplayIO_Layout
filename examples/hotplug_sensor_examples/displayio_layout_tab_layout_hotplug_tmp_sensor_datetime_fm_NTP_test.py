...
if use_ntp:
    print(
        "\nTabLayout test with I2C Temperature sensor and NTP synchronized local time"
    )
else:
...
if use_ntp:
    ...
    if ntp.valid_time:
        ...
        # Get the current time in seconds since Jan 1, 1970 and correct it
        # for local timezone (defined in secrets.h)
        ...
        default_dt = time.localtime(current_time)
        if my_debug:
            print(
                "Datetime (struct_time) synchronized from NTP pool, now =", default_dt
            )
# -----------------------------------
...
def handle_dt(dt):
    global dt_refresh, sDT_old, pge3_lbl, pge3_lbl2, pge3_lbl3, o_secs, c_secs
    RetVal = False
    s = "Date/time: "
    sYY = str(dt[yy])
    sMO = (
...
def get_dt():
    global rtc_present, rtc, pge3_lbl, pge3_lbl2, pge3_lbl3
    dt = None
    RetVal = False
...
def setup():
    ...
    if my_debug:
        print(
            "default_dt (from NTP-server - set to the internal RTC: {}".format(
                default_dt
            )
        )
